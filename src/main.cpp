...
int main() {
    ...
    // --------------------
    GLFWwindow *window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Chess and Lamp", NULL, NULL);
    if (window == NULL) {
    ...
    Shader simpleDepthShader("resources/shaders/shadow_mapping_depth.vs", "resources/shaders/shadow_mapping_depth.fs", "resources/shaders/shadow_mapping_depth.gs");

    // load models
    // -----------
    Model chessModel("resources/objects/Chess/Chessset.obj");
...
void DrawImGui(ProgramState *programState) {
...

    {
        ...
        ImGui::DragFloat3("Backpack position", (float*)&programState->chessPosition);
        ImGui::DragFloat("Backpack scale", &programState->chessScale, 0.05, 0.1, 4.0);
...
void drawChess(Shader ourShader, Model ourModel) {
    ...
    model = glm::translate(model, programState->chessPosition);
    model = glm::scale(model, glm::vec3(programState->chessScale));
